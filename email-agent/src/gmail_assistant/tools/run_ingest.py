    ...
    if token_data is None:
        if TOKEN_PATH.exists():
            try:
                ...
                print(f"🎟️ Using token from {TOKEN_PATH}")
            except Exception as e:
...
async def ingest_email_to_langgraph(email_data, graph_name, url="http://127.0.0.1:2024"):
    ...
    print(f"🔗 Connecting to LangGraph at {url}")
    client = get_client(url=url)
    if not graph_name or not graph_name.strip():
        print("❗️ No graph name provided. Please pass --graph-name to select a deployed graph.")
        raise ValueError("Graph name is required to create a run")
    ...
    print(f"📝 Updating thread metadata for {thread_id} with email_id={email_data['id']}")
    ...
    print(
        "📦 Run payload preview:",
        {
            ...
            "body_len": len(email_data.get("page_content", "")),
        },
    )
    ...
    try:
        ...
        print("❌ Failed to create run. Details:")
        print(f"  - thread_id: {thread_id}")
        print(f"  - graph_name: '{graph_name}'")
        print(f"  - subject: {email_data['subject']}")
        print(f"  - email_id: {email_data['id']}")
        print(f"  - error: {str(e)}")
        raise
...
async def fetch_and_process_emails(args):
    ...
    try:
        ...
        if args.minutes_since > 0:
            ...
            query += f" after:{after}"
            try:
                human_time = datetime.fromtimestamp(after)
                print(f"🕒 Time filter: last {args.minutes_since} minutes → after {after} ({human_time})")
            except Exception:
                print(f"🕒 Time filter: last {args.minutes_since} minutes → after {after}")
        ...
        if not messages:
            print("❗️ No emails found matching the criteria")
            return 0
            
        print(f"ℹ️ Found {len(messages)} emails")
        ...
        for i, message_info in enumerate(messages):
            ...
            print(f"Subject: {email_data['subject']}")
            print(
                f"Email content length: {len(email_data.get('page_content', ''))} chars; Gmail ID: {email_data['id']}; Thread ID: {email_data['thread_id']}"
            )
